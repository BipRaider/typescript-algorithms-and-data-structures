...
class VisitMetadata {
  ...
  constructor({ discoveryTime, lowDiscoveryTime }: any) {
 ...
 * @return {Object}
 */
export default function articulationPoints(graph: Graph): object {
  // Set of vertices we've already visited during DFS.
  const visitedSet: Record<string, any> = {};

  // Set of articulation points.
  const articulationPointsSet: Record<string, any> = {};

  // Time needed to discover to the current vertex.
  let discoveryTime: number = 0;
  ...
  const dfsCallbacks = {
    ...
    leaveVertex: ({ currentVertex, previousVertex }: { currentVertex: GraphVertex; previousVertex: GraphVertex }) => {
      ...
      visitedSet[currentVertex.getKey()].lowDiscoveryTime = currentVertex
         ...
         */
        .reduce((lowestDiscoveryTime: number, neighbor: { getKey: () => string | number }) => {
