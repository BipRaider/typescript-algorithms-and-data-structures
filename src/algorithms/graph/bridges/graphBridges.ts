...
class VisitMetadata {
  discoveryTime: any;
  lowDiscoveryTime: any;

  constructor({ discoveryTime, lowDiscoveryTime }: { discoveryTime: any; lowDiscoveryTime: any }) {
 ...
 * @return {Object}
 */
export default function graphBridges(graph: Graph): Record<string, any> {
  ...
  const dfsCallbacks = {
     ...
     */
    enterVertex: ({ currentVertex }: { currentVertex: GraphVertex; previousVertex: GraphVertex }) => {
      // Tick discovery time.
      discoveryTime += 1;
    ...
    leaveVertex: ({ currentVertex, previousVertex }: { currentVertex: GraphVertex; previousVertex: GraphVertex }) => {
      ...
      visitedSet[currentVertex.getKey()].lowDiscoveryTime = currentVertex
        .getNeighbors()
        .filter((earlyNeighbor: { getKey: () => any }) => earlyNeighbor.getKey() !== previousVertex.getKey())
        .reduce(
          /**
           * @param {number} lowestDiscoveryTime
           * @param {GraphVertex} neighbor
           */
          (lowestDiscoveryTime: number, neighbor: { getKey: () => string | number }) => {
