...
function isSafe(
  ...
  const candidateDuplicate = cycle.find(
    (vertex: { getKey: () => any }) => vertex.getKey() === vertexCandidate.getKey(),
  );

  return !candidateDuplicate;
}

function isCycle(adjacencyMatrix: number[][], verticesIndices: Record<string, any>, cycle: GraphVertex[]): boolean {
