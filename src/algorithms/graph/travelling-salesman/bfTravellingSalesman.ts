...
function findAllPaths(
  ...
  const visitedSet: Record<string, any> = currentPath.reduce((accumulator, vertex) => {
    const updatedAccumulator: Record<string, any> = { ...accumulator };
  ...
  const unvisitedNeighbors = startVertex.getNeighbors().filter((neighbor: GraphVertex) => {
...
function getCycleWeight(
  ...
  verticesIndices: Record<string, any>,
  cycle: GraphVertex[],
): number {
  let weight = 0;
