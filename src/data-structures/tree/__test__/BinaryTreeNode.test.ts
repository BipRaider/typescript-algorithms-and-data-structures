...
describe('BinaryTreeNode', () => {
  ...
  it('should replace child node', () => {
    ...
    expect(rootNode.replaceChild(rootNode.right, rootNode.right?.right || null)).toBeTruthy();
    expect(rootNode.right?.value).toBe(5);
    expect(rootNode.right?.right).toBeNull();
    expect(rootNode.traverseInOrder()).toEqual([1, 2, 5]);

    expect(rootNode.replaceChild(rootNode.right, rootNode.right?.right || null)).toBeFalsy();
    expect(rootNode.traverseInOrder()).toEqual([1, 2, 5]);
